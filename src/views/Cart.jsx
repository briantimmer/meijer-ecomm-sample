...
export const Cart = (props) => {
  const { cartItems, cartTotal, removeFromCart, clearCart } = useContext(
    GlobalContext
  );
        ...
        <Col>
              ...
              <tbody>
                {cartItems.map((item, index) => (
                  <tr key={index} className="cart-line-item">
                    ...
                    <td className="narrow-col">
                      {formatAsCurrency(item.lineTotal)}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <Modal isOpen={checkoutModal} toggle={toggleCheckoutModal}>
            ...
            <ModalBody className="text-center">
              <Button className="my-3" color="primary" onClick={startOverClick}>
                Go pick up some more produce.{" "}
                <FontAwesomeIcon icon={Icons.faThumbsUp} />
              </Button>
            </ModalBody>
          </Modal>
        </Col>
